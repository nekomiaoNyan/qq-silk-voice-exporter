...
QQ 目录中的文件扩展名经常是 `.amr`，但实际内容可能是腾讯前缀 `0x02` 加 `#!SILK_V3` 的 SILK 数据。本项目的原生程序 `qq-silk.exe` 负责把这类文件解码为 24 kHz、16-bit、单声道 WAV；`wechat-record.exe` 使用 Windows WASAPI 回环录制用户主动播放的系统声音；`wechat-voice.exe` 使用 Windows 自带的 `winsqlite3.dll`，从用户明确选择的、已解密微信 `media_*.db` 副本中只读导出 `VoiceInfo.voice_data`。可阅读的 PowerShell 脚本负责图形界面、筛选和批量转换。
...
- 一次选择多个文件，或添加整个文件夹；
- 在微信 4.x 中播放语音并保存为本地 WAV，不读取微信数据库；
...
1. 主界面点击“微信语音”，选择“播放录音（推荐）”。
2. 关闭音乐、视频和通知声等其他声音，点击“开始录音”。
3. 回到微信播放目标语音；播放结束后点击“停止并保存”。
4. 工具通过 Windows WASAPI 把默认扬声器的系统混音保存为 WAV。文件名默认只含日期时间，不含联系人或群名称。

录音期间的**所有系统声音**都会进入文件，所以一次只播放目标语音。此方式需要逐条播放，音频也经过了微信播放链路；它的优点是不读取微信文件、聊天数据库、进程内存、账号信息或密钥。
...
为保持低误报、可审计和版本稳定性，发布包不会扫描 `Weixin.exe` 进程内存，也不会自动获取或保存数据库密钥。GitHub 于 2026-07-13 公布的一份 [DMCA 下架通知](https://github.com/github/dmca/blob/master/2026/07/2026-07-13-wechat.md) 显示，同类密钥提取/解密项目及其仓库网络已被下架；该页面记录的是权利方主张和平台处理，并不是法院判决。无论如何，本项目不会发布这类组件。误选仍加密的官方数据库时，界面会用中文说明可行方案并明确拒绝，不会修改它。

播放录音器也可单独使用；开始后按 Enter 停止并写完 WAV 文件头：
