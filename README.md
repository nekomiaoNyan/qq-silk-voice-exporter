...
在 Windows 上导入和转换 QQ SILK V3 语音，并从**已解密的微信 4.x 媒体数据库副本**中只读提取语音。项目只包含可审计源码和可复现构建流程，不包含 AutoIt、UPX、安装器、自解压程序、遥测或联网功能。
...
QQ 目录中的文件扩展名经常是 `.amr`，但实际内容可能是腾讯前缀 `0x02` 加 `#!SILK_V3` 的 SILK 数据。本项目的原生程序 `qq-silk.exe` 负责把这类文件解码为 24 kHz、16-bit、单声道 WAV；`wechat-voice.exe` 使用 Windows 自带的 `winsqlite3.dll`，从用户明确选择的、已解密的微信 `media_*.db` 副本中只读导出 `VoiceInfo.voice_data`。可阅读的 PowerShell 脚本负责图形界面、筛选和批量转换。
...
2. 解压全部文件，双击 `Start-VoiceConverter.cmd`（旧的 `Start-QQSilkConverter.cmd` 仍可使用）。
3. QQ 用户点击“添加文件”或拖入文件/文件夹；微信用户点击“导入微信 DB”并选择已解密的 `media_*.db` 副本。选择参数后点击“开始转换”。
...
微信 4.x 的语音不是独立文件，而是保存在加密数据库的 `VoiceInfo` 表中。本项目当前支持**已解密的 SQLite 数据库副本**，不直接读取仍加密的官方 `media_*.db`：

1. 使用你信任、可审计的本地方式制作一份已解密数据库副本；不要把数据库、密钥或聊天样本放进本仓库。
2. 在图形界面点击“导入微信 DB”，选择副本。
3. 可按日期筛选并限制导入条数。程序先把原始语音保存为数字编号的 `.silk`，不在文件名中写入联系人或群名称，然后加入转换列表。
4. 选择 WAV/MP3 和输出目录，点击“开始转换”。

为保持低误报、可审计和版本稳定性，发布包不会扫描 `Weixin.exe` 进程内存，也不会自动获取或保存数据库密钥。此类行为容易触发安全软件，并会随着微信版本变化失效。误选仍加密的官方数据库时，界面会明确拒绝，不会修改它。
