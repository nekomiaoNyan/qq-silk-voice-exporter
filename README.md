...
> **最简单的使用方式：** 从 [Releases](https://github.com/nekomiaoNyan/qq-silk-voice-exporter/releases/latest) 下载 `QQ-WeChat-SILK-Voice-Converter-v1.3.1-windows-x64.zip`，**完整解压**，然后只需双击根目录中的 **`Start-VoiceConverter.exe`**。普通用户不需要打开 `bin` 文件夹，也不要双击里面的三个命令行组件。
...
1. 从 [Releases](https://github.com/nekomiaoNyan/qq-silk-voice-exporter/releases/latest) 下载 `QQ-WeChat-SILK-Voice-Converter-v1.3.1-windows-x64.zip`。普通用户不需要安装编译器。
2. 完整解压 ZIP，双击根目录里唯一需要打开的程序 `Start-VoiceConverter.exe`。它只显示转换器图形界面，不会弹出黑色 CMD 窗口；启动失败时会弹窗说明。
3. QQ 用户点击“添加文件”或拖入文件/文件夹；微信用户点击“微信语音”，再选择“播放录音（推荐）”或“已解密 DB（高级）”。

压缩包根目录只放一个程序入口 `Start-VoiceConverter.exe`。`bin` 文件夹里的 `qq-silk.exe`、`wechat-record.exe` 和 `wechat-voice.exe` 是图形界面自动调用的命令行组件，普通用户无需直接运行。
...
.\bin\wechat-record.exe record-process $wechat.Id 'D:\微信语音\wechat-only.wav'

# 仅用于不支持按进程录音的旧系统：
.\bin\wechat-record.exe record 'D:\微信语音\wechat-voice.wav'
```
...
```powershell
.\bin\wechat-voice.exe check 'D:\微信数据库副本\media_0.db'
.\bin\wechat-voice.exe export 'D:\微信数据库副本\media_0.db' 'D:\微信语音\Raw' --limit 1000
```
...
```powershell
.\bin\qq-silk.exe '输入.amr' '输出.wav'
.\bin\qq-silk.exe '输入.slk' '输出.wav' --sample-rate 24000
```
