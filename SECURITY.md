...
This project intentionally performs no network access, telemetry, persistence, privilege elevation, process-memory scanning, key extraction, or source-file deletion. It decodes user-selected local files into a separate output directory. WeChat database support opens only a user-selected, decrypted SQLite copy in read-only mode; encrypted official databases are rejected. The optional playback recorder starts only after an explicit user click, captures the default Windows playback-device mix through WASAPI, and writes only to the user-selected WAV path. It does not inspect which application produced the audio, but every system sound played during that interval can be present in the recording.
